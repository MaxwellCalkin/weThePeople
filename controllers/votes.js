...
module.exports = {
  ...
  getSearch: async (req, res) => {
    ...
    try {
      const resp = await fetch(
        `https://api.propublica.org/congress/v1/bills/search.json?query=${req.query.search}`,
        {
          headers: {
  ...
  getDetails: async (req, res) => {
    try {
      const resp = await fetch(
        `https://api.propublica.org/congress/v1/${req.params.congress}/bills/${req.params.bill_slug}.json`,
        {
          headers: {
            "X-API-KEY": `${process.env.CONGRESS_KEY}`,
          },
      ...
      const billExists = await Bill.exists({
        billSlug: data.results[0].bill_id,
      });

      let votes = 0;

      await Bill.findOne(
        { billSlug: data.results[0].bill_slug },
        function (err, bill) {
          if (err) {
            console.log(err);
          } else if (bill) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.propublica.org/congress/v1/members/${
          bill.bill_type === "hr" ? "house" : "senate"
        }/${req.user.state.toUpperCase()}/${
          bill.bill_type === "hr" ? req.user.cd + "/" : ""
        }current.json`,
        {
          headers: {
            "X-API-KEY": `${process.env.CONGRESS_KEY}`,
      ...
      async function findVotesIndex(repVotesData) {
        const voteIndex = await repVotesData.results[0].votes.forEach(
          (vote, index) => {
      ...
      const firstRepVotesResponse = await fetch(
        `https://api.propublica.org/congress/v1/members/${repsArray[0].id}/votes.json`,
        {
          headers: {
            "X-API-KEY": `${process.env.CONGRESS_KEY}`,
          },
      ...
      const firstRepVotesDataOnThisBill =
        firstRepVotesData.results[0].votes.filter(
          (x) => x.bill.bill_id === bill.bill_id
        );

      const firstRepsVote =
        firstRepVotesDataOnThisBill.length > 0
          ? firstRepVotesDataOnThisBill[0].position
      ...
      if (repsArray.length > 1) {
        const secondRepVotesResponse = await fetch(
          `https://api.propublica.org/congress/v1/members/${repsArray[1].id}/votes.json`,
          {
            headers: {
              "X-API-KEY": `${process.env.CONGRESS_KEY}`,
            },
        ...
        const secondRepVotesDataOnThisBill =
          secondRepVotesData.results[0].votes.filter(
            (x) => x.bill.bill_id === bill.bill_id
          );

        secondRepsVote =
          secondRepVotesDataOnThisBill.length > 0
            ? secondRepVotesDataOnThisBill[0].position
            : "Has Not Voted On This Bill";
  ...
  createYea: async (req, res) => {
    ...
    console.log(
      "Bill exists?: ",
      await Bill.exists({ billSlug: req.body.billSlug })
    );
    try {
